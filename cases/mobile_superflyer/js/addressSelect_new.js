		 var sortItem = function(el){
				var arr = [];
				el.find('li').each(function(){
				arr.push($(this).html()+'@@'+($(this).attr('value')||' '))
				});
				...
				for(i=0;i<arr.length;i++){
					...
					if(temp[0] == '北京' || temp[0] == '重庆' || temp[0] == '天津' || temp[0] == '上海'){
						temp[0] += '市';
					}
					html += '<li value="' + temp[1] + '">' + temp[0] + '</li>';
				}
				el.html(html);	 
	      }		  
		  $(function(){
			   var residenceAddress = new addressSelector($("#province"),$("#city"));  //省市二级联动
			   sortItem($("#province"))
		   });
            var addressSelector = function(province,city){
            ...
            province.delegate('li','click',function(){
				province.fadeOut(100);
                var changeThis = $(this).attr('value');
				var html = $(this).html();
                theThis.changeAction(city,theThis.data,changeThis,html);				 
				 
			 });    
            city.delegate('li','click',function(){
				var html = $(this).html();
                alert(html+'  ps:此处增加自定义方法(addressSelect_new.js)');//heloo developers 这里增加自定义方法		  
			 });   			 

        }
        addressSelector.prototype.changeAction = function(elem,data,changeThis,html){	
			$('h6#title').html('选择隶属于 '+html+' 的城市');
			var back = function(){
			   elem.html('');
               $('#province').fadeIn(100);
			   $('h6#title').html('选择省份');
			}
			$('a.address').bind('click',back)
            var thisVal = changeThis;
            elem.html('');
            var j = 0;
            for(var i in data){
                 if(thisVal == data[i][1]){
				  var temp = '<li>' + data[i][0] + '</li>'
                  elem.append(temp)
                 }
            }
			sortItem(elem)
        }
        addressSelector.prototype.data = {
            "110000": ["\u5317\u4eac", "1"],
            "110100": ["\u5317\u4eac\u5e02", "110000"],
            "110101": ["\u4e1c\u57ce\u533a", "110100"],
            "110102": ["\u897f\u57ce\u533a", "110100"],
