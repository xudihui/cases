...
(function($) {
	$.fn.teninedialog = function(options) {
		...
		return this.each(function() {
			...
			var tmpHtml = '<div class="modal fade" id="{ID}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel_{ID}" aria-hidden="true"><div class="modal-dialog zoomInDown"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title" id="myModalLabel_{ID}">{title}</h4></div><div class="modal-body" id="body_{ID}" style="overflow-y: auto;max-height: 600px;">{body}</div><div class="modal-footer"><img id="ajax-loader" style="display:none" height="32" width="32" src="' + loadimg + '" />{button}</div></div></div></div>';
			...
			//增加单个button宽度为100%
			modalObj.find('.btn').length == 1 ?  modalObj.find('.btn').css('width','100%') : null;
			
			//绑定本身的事件
			modalObj.find('.btn').first().bind('click',function(){
				modalObj.remove();
			})
		});
...
window.showDialog = function(content, title) {
	$.teninedialog({
		title: title || "系统提示",
		content: content
	})
...
window.showConfirm = function(config) {
	...
	var ok = config.ok ||
	function() {};
	var ok_argument = config.ok_argument || [""];
	var cancel = config.cancel ||
	function() {};
	var cancel_argument = config.cancel_argument || [""];
	$.teninedialog({
		title: title || "系统提示",
		content: content,
		...
		clickButton: function(sender, modal, index) {
			...
			if (index == 0) {
				...
				if (typeof(cancel) == "function") {
					try {
						cancel.apply(this, cancel_argument)
					} catch (e) {
						console.log(e.name + ": " + e.message)
					}
				}
